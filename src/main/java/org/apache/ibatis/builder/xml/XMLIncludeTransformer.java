...
public class XMLIncludeTransformer {
  ...
  public void applyIncludes(Node source) {
    if (source.getNodeName().equals("include")) {
      ...
      String columnAliasPrefix = getOptionalStringAttribute(source, "columnPrefix");
      applyIncludes(toInclude);
      if (toInclude.getOwnerDocument() != source.getOwnerDocument()) {
        toInclude = source.getOwnerDocument().importNode(toInclude, true);
      }
  ...
  private String transformColumnList(String columns, String tableAlias, String columnPrefix) {
    ...
    for (int i = 0; i < tokens.length; i++) {
      ...
      if (!token.toLowerCase().contains(" ")) {
        stringBuilder.append(" as ").append(columnPrefix).append(token);
      }
